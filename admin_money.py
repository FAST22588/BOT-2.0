...
class AdminMoney(commands.Cog):
    ...
    @app_commands.describe(user_id="ID ของผู้ใช้", amount="จำนวนเงินที่ต้องการเพิ่ม")
    async def us_command(self, interaction: discord.Interaction, user_id: str, amount: int):
        ...
        await interaction.response.send_message(
            f"เพิ่มเงิน **{amount}** ให้ผู้ใช้ `{user_id}` แล้ว", ephemeral=True
        )

    @app_commands.command(name="delete", description="ลบเงินจากผู้ใช้ตาม ID")
    @app_commands.describe(user_id="ID ของผู้ใช้", amount="จำนวนเงินที่ต้องการลบ")
    async def delete_command(self, interaction: discord.Interaction, user_id: str, amount: int):
        ...
        await interaction.response.send_message(
            f"ลบเงิน **{amount}** จากผู้ใช้ `{user_id}` แล้ว", ephemeral=True
        )

async def setup(bot):
    cog = AdminMoney(bot)
    await bot.add_cog(cog)
    bot.tree.add_command(cog.us_command)
    bot.tree.add_command(cog.delete_command)
